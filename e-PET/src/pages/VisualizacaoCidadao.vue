<template>
  <v-container v-if="citizen" fluid class="pa-0 bg-background min-h-screen">
      ...
      <div class="d-flex align-center flex-wrap gap-4">
        ...
        <div class="flex-grow-1">
          <div class="d-flex align-center flex-wrap gap-2">
            <h1 class="text-h4 font-weight-black text-grey-darken-4 mb-0">{{ citizen.name }}</h1>
            <v-chip
          ...
          <div class="text-subtitle-1 text-grey-darken-1 font-weight-medium mt-1">
            {{ citizen.sex }} • {{ calculateAge(citizen.birthDate) }} anos • {{ formatDate(citizen.birthDate) }}
          </div>
        </div>

        <div class="text-right d-none d-md-block">
          <div class="text-caption text-grey font-weight-bold uppercase letter-spacing-1">Última Atualização</div>
          <div class="text-h6 font-weight-black text-primary">{{ formatDate(citizen.lastUpdate || '') }}</div>
        </div>
      </div>

      <!-- Quick Info Strip -->
      <v-row class="mt-6 info-strip pa-3 rounded-lg bg-grey-lighten-4" no-gutters>
        ...
        <v-col cols="6" sm="4" md="3" class="px-2 border-r">
          ...
          <div class="value text-truncate">{{ citizen.motherName || '---' }}</div>
        </v-col>
        <v-col cols="6" sm="4" md="2" class="px-2 border-r">
        ...
        <v-col cols="12" sm="4" md="3" class="px-2">
          ...
          <div class="value text-truncate">{{ citizen.responsibleUnit || '---' }}</div>
        </v-col>
      </v-row>
    </v-card>
    ...
    <div class="bg-white border-b">
      <v-tabs v-model="tab" color="primary" align-tabs="start" class="px-4 custom-tabs">
        <v-tab value="info" prepend-icon="mdi-information">Informações</v-tab>
        <v-tab value="cover" prepend-icon="mdi-card-account-details">Folha de Rosto</v-tab>
      </v-tabs>
    </div>
        ...
        <v-row>
            ...
            <v-card border class="mb-6 pa-6">
              ...
              <v-row class="details-grid">
                ...
                <v-col cols="12" sm="6" md="4">
                  ...
                  <div class="detail-value">{{ formatDate(citizen.birthDate) }}</div>
                </v-col>
                <v-col cols="12" sm="6" md="4">
                ...
                <v-col cols="12" sm="6" md="4">
                  ...
                  <div class="detail-value">{{ citizen.nationality || 'Brasileira' }}</div>
                </v-col>
              </v-row>
            </v-card>
          ...
          <v-col cols="12" lg="4">
            <v-card border class="pa-6 bg-primary-lighten-5 mb-6">
              <h3 class="text-subtitle-1 font-weight-black text-primary mb-4 uppercase letter-spacing-1">
                Equipe Responsável
              </h3>
              <div class="mb-4">
                <div class="detail-label">Equipe</div>
                <div class="detail-value text-primary">{{ citizen.responsibleTeam || 'Não atribuída' }}</div>
              </div>
              <div>
                <div class="detail-label">Unidade</div>
                <div class="detail-value">{{ citizen.responsibleUnit || '---' }}</div>
              </div>
            </v-card>
            ...
            >
              As informações de equipe são baseadas no cadastro individual mais recente.
            </v-alert>
          </v-col>
        </v-row>
      </v-window-item>
        ...
        <v-row>
          <v-col cols="12" md="4">
            <v-card border class="health-card">
              <div class="health-card-header bg-red-lighten-5 text-red-darken-4 pa-4 d-flex align-center">
                <v-icon icon="mdi-alert-octagon" class="mr-2"></v-icon>
                <span class="font-weight-black">LISTA DE PROBLEMAS</span>
                <v-spacer></v-spacer>
                <v-chip size="x-small" color="red-darken-4" class="font-weight-bold">{{ healthConditions.length }}</v-chip>
              </div>
              <v-list class="pa-0">
                <v-list-item
                ...
                >
                  <template v-slot:prepend>
                    <v-icon :color="cond.isFamily ? 'orange-darken-2' : 'red'" size="small" :icon="cond.isFamily ? 'mdi-account-group-outline' : 'mdi-circle-medium'"></v-icon>
                  </template>
                  <v-list-item-title class="text-body-2 font-weight-bold text-grey-darken-3">
                    {{ cond.label }}
                  </v-list-item-title>
                ...
                <div v-if="allConditions.length === 0" class="pa-10 text-center">
                  <v-icon icon="mdi-check-circle-outline" color="green-lighten-3" size="48" class="mb-2"></v-icon>
                  <div class="text-caption text-grey italic">Nenhuma condição ativa registrada</div>
                </div>
              </v-list>
            </v-card>
          </v-col>
          ...
          <v-col cols="12" md="4">
            <v-card border class="health-card">
              <div class="health-card-header bg-blue-lighten-5 text-blue-darken-4 pa-4 d-flex align-center">
                <v-icon icon="mdi-pill" class="mr-2"></v-icon>
                <span class="font-weight-black">MEDICAMENTOS ATIVOS</span>
              </div>
              <div class="pa-12 text-center">
                <v-icon icon="mdi-pill-off" color="grey-lighten-2" size="48" class="mb-2"></v-icon>
                <div class="text-caption text-grey italic">Nenhum medicamento em uso</div>
              </div>
            </v-card>
          </v-col>
          ...
          <v-col cols="12" md="4">
            <v-card border class="health-card">
              <div class="health-card-header bg-orange-lighten-5 text-orange-darken-4 pa-4 d-flex align-center">
                <v-icon icon="mdi-test-tube" class="mr-2"></v-icon>
                <span class="font-weight-black">EXAMES RECENTES</span>
              </div>
              <div class="pa-12 text-center">
                <v-icon icon="mdi-flask-empty-outline" color="grey-lighten-2" size="48" class="mb-2"></v-icon>
                <div class="text-caption text-grey italic">Sem registros nos últimos 6 meses</div>
              </div>
            </v-card>
          </v-col>
        </v-row>
    ...
    <v-dialog v-model="dialog" width="500">
      <v-card v-if="selectedCondition" class="overflow-hidden">
        <v-toolbar color="red-darken-1" flat>
          <v-toolbar-title class="font-weight-black">{{ selectedCondition.label }}</v-toolbar-title>
          <v-btn icon="mdi-close" @click="dialog = false"></v-btn>
        </v-toolbar>
        <v-card-text class="pa-6">
          <div class="mb-6">
            <div class="detail-label mb-1">Identificado em</div>
            <div class="text-h6">{{ formatDate(selectedCondition.updatedAt || '') }}</div>
          </div>
          <div class="mb-6">
            <div class="detail-label mb-1">Classificação</div>
            <v-chip color="red" variant="tonal" class="font-weight-bold">{{ selectedCondition.type || 'Condição de Saúde' }}</v-chip>
          </div>
        ...
        <v-card-actions class="pa-4 bg-grey-lighten-5">
          <v-spacer></v-spacer>
          <v-btn color="grey-darken-1" variant="text" @click="dialog = false">Fechar</v-btn>
  ...
  <v-container v-else class="fill-height justify-center bg-background">
    <div class="text-center">
      ...
      <div class="text-h6 font-weight-bold text-grey">Carregando Prontuário...</div>
    </div>
  </v-container>
</template>
...
const effectiveRisk = computed(() => {
  if (citizen.value?.riskClass && citizen.value.riskClass !== 'Sem Risco' && citizen.value.riskClass !== 'Sem Classificação') {
    return { label: citizen.value.riskClass, source: 'individual' };
  }
  if (family.value?.riskClass && family.value.riskClass !== 'Sem Risco' && family.value.riskClass !== 'Sem Classificação') {
    return { label: family.value.riskClass, source: 'família' };
  }
  return { label: citizen.value?.riskClass || 'Sem Classificação', source: 'none' };
});
...
const familySentinels = computed(() => {
  ...
  const check = (count: number | undefined, label: string, type: string = 'Risco da Família') => {
    if (count && count > 0) {
      sentinels.push({
        id: `family-${label}`,
        label: `${label}`,
  ...
  if (f.basicSanitation === false) {
    sentinels.push({
      ...
      value: 'Baixas condições de saneamento identificadas no domicílio da família.',
      isFamily: true
    });
  }
